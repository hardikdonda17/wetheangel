...
(function($) {
    ...
    function formValidator() {
        const Name = $('#patient_name'),
            ...
            Amount = $('#amount'),
            Payment_btn = $('#paynow_btn');

        $('#payment_form').on('submit', function(e) {
            e.preventDefault();
            var patient_name = Name.val();
            var patient_ref = Reference.val();
            var amount = Amount.find(":selected").val();
            var count = 0;
            if (patient_name == '') {
                Name.prev().css("opacity", "1");
                count++;
            } else {
                Name.prev().css("opacity", "0");
                count--;
            }
            if (patient_ref == '') {
                Reference.prev().css("opacity", "1");
                count++;
            } else {
                Reference.prev().css("opacity", "0");
                count--;
            }
            if (amount == '' && amount < 50000) {
                Amount.next().next().css("opacity", "1");
                count++;
            } else {
                Amount.next().next().css("opacity", "0");
                count--;
            }
            if (patient_name != '' && patient_ref != '' && amount != '' && amount >= 50000) {
                $('#payment_form').submit();
            }
        });
    ...
    // ==== Preloader
    function preloader() {
        $('#preloader').delay(300).fadeOut(3000);
    }
...
////////////////////// Servicve click event start //////////////////

document.getElementsByClassName('.iconic-box-two')[0]
    .addEventListener('click', function(event) {
        window.open("../surgery.html", "_blank");
    });
document.getElementsByClassName('.iconic-box-two')[1]
    .addEventListener('click', function(event) {
        window.open("../hotel.html", "_blank");
    });
document.getElementsByClassName('.iconic-box-two')[2]
    .addEventListener('click', function(event) {
        window.open("../transportation.html", "_blank");
    });
document.getElementsByClassName('.iconic-box-two')[3]
    .addEventListener('click', function(event) {
        window.open("../pricing.html", "_blank");
    });


////////////////////// Servicve click event end //////////////////