// Transpotation road map js start\\
/*
 * Application state
 */
const state = {
...
function drawLine(theBox) {
    ...
    setTimeout(function() { connector.style.width = distanceBetweenItemAndLine + 'px'; }, 300);
}
    ...
    if (x <= (window.innerHeight - item.offsetHeight)) {
        ...
        if (item.classList.contains('__goals')) {
            ...
            item.children[1].style.opacity = '1'
                //Then the rest
            setTimeout(() => { item.children[2].style.opacity = '1'; }, 300);
